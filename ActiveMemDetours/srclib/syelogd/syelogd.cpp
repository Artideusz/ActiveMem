...
BOOL CloseConnection(PCLIENT pClient)
{
    ...
    if (pClient != NULL) {
        if (pClient->hPipe != INVALID_HANDLE_VALUE) {
            ...
            FlushFileBuffers(pClient->hPipeMonitorServer);
            if (!DisconnectNamedPipe(pClient->hPipe)) {
                MyErrExit("DisconnectNamedPipe");
            }
            if (!DisconnectNamedPipe(pClient->hPipeMonitorServer)) {
                MyErrExit("DisconnectNamedPipe (Monitor Server)");
            }
            CloseHandle(pClient->hPipe);
            CloseHandle(pClient->hPipeMonitorServer);
            pClient->hPipe = INVALID_HANDLE_VALUE;
            pClient->hPipeMonitorServer = INVALID_HANDLE_VALUE;
        }
        GlobalFree(pClient);
        pClient = NULL;
    }
...
PCLIENT CreatePipeConnection(HANDLE hCompletionPort)
{
    ...
    HANDLE hPipeMonitorServer = NULL;

    while (1) 
    { 
        hPipeMonitorServer = CreateFileW(L"\\\\.\\pipe\\active_mem_monitor",   // pipe name
            GENERIC_WRITE, 
            0,              // no sharing 
        ...
        if (!WaitNamedPipeW(L"\\\\.\\pipe\\active_mem_monitor", 20000)) 
        { 
            MyErrExit("Could not open pipe: 20 second wait timed out.");
        } 
    }
    ...
    // WriteFile( 
    //     hPipeMonitorServer,     // pipe handle 
    //     "HELLO FROM SYELOGD",   // message 
    //     19,                     // message length 
    ...
    pClient->hPipe = hPipe;
    pClient->hPipeMonitorServer = hPipeMonitorServer;
...
BOOL SendMessageToMonitorServer(PCLIENT pClient) {
    PSYELOG_MESSAGE pMessage = &pClient->Message;
    PCHAR szMsg = pMessage->szMessage;
    ...
    *szMsg = '\0';
    ...
    WriteFile( 
        pClient->hPipeMonitorServer,    // pipe handle 
        pMessage->szMessage,            // message 
        strlen(pMessage->szMessage),    // message length 
        &cbWritten,                     // bytes written 
        NULL);                          // not overlapped
...
DWORD WINAPI WorkerThread(LPVOID pvVoid)
{
    ...
    for (BOOL fKeepLooping = TRUE; fKeepLooping;) {
        ...
        if (pClient->fAwaitingAccept) {
            ...
            // SendMessageToMonitorServer(pClient);
